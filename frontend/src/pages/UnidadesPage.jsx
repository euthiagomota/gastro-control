...
export default function UnidadesPage() {
  ...
  const loadUnidades = async () => {
    try {
      setLoading(true);
      const data = await unidadeService.listUnidades();
      setUnidades(data || []);
    } catch (err) {
  ...
  const colors = ['from-primary-50 to-primary-100', 'from-violet-50 to-violet-100', 'from-green-50 to-green-100'];
  const emojis = ['🏪', '🍽️', '🚚'];

  const unidadesFormatted = unidades.map((unidade, idx) => ({
    ...unidade,
    emoji: emojis[idx % emojis.length],
    color: colors[idx % colors.length],
    status: unidade.ativo ? 'aberto' : 'fechado',
  }));
                  ...
                  <div className="p-5">
                    ...
                    <div className="space-y-1.5 text-sm text-gray-500">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{unidade.localizacao || 'Sem localização'}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Users size={14} className="text-primary-600 flex-shrink-0" />
                        <span>{unidade.quantidadeFuncionarios || 0} funcionários</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Building2 size={14} className="text-primary-600 flex-shrink-0" />
                        <span>{unidade.tipo || 'Restaurante'}</span>
                      </div>
                    </div>
