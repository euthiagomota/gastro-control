...
export default function FuncionarioPerfil() {
  ...
  const loadPerfil = async () => {
    try {
      ...
      const userStats = await usuarioService.getUserStats(user.id);
      setStats(userStats);
    } catch (err) {
  ...
  const infoItems = [
    { icon: Mail, label: 'Email', value: funcionario.email || 'N/A' },
    { icon: MapPin, label: 'Unidade', value: funcionario.unidade || 'N/A' },
    { icon: Briefcase, label: 'Cargo', value: funcionario.cargo || 'Operador' },
    { icon: Clock, label: 'Horário', value: funcionario.horario || 'Não especificado' },
  ];

  const defaultStats = [
    { label: 'Tarefas completas', value: stats?.tarefasCompletas || '0', bg: 'bg-blue-50', border: 'border-blue-200', color: 'text-blue-700' },
    { label: 'Taxa de conclusão', value: stats?.taxaConclusao || '0%', bg: 'bg-green-50', border: 'border-green-200', color: 'text-green-700' },
    { label: 'Dias consecutivos', value: stats?.diasConsecutivos || '0', bg: 'bg-purple-50', border: 'border-purple-200', color: 'text-purple-700' },
    { label: 'Avaliação média', value: stats?.avaliacao || '0', bg: 'bg-amber-50', border: 'border-amber-200', color: 'text-amber-700' },
  ];
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <Card className="text-center">
        <div className="flex flex-col items-center py-2">
          ...
          <p className="text-sm text-gray-500 mb-3">{funcionario.cargo || 'Operador'}</p>
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-primary-50 border border-primary-200 rounded-full text-xs font-semibold text-primary-700">
            <span className={`w-2 h-2 rounded-full ${funcionario.ativo ? 'bg-green-500' : 'bg-red-500'}`} />
            {funcionario.ativo ? 'Ativo' : 'Inativo'} · {funcionario.unidade || 'Sem unidade'}
          </span>
        </div>
      </Card>
        ...
        <div className="space-y-2">
          {quickLinks.map((link, i) => {
            ...
            return (
              <button
                key={i}
                className="w-full flex items-center gap-3 p-3.5 rounded-xl border border-gray-200 hover:border-primary-300 hover:bg-primary-50 transition-all text-left group"
              >
      ...
      <Card className="bg-blue-50 border-blue-200">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <button className="text-xs text-blue-700 font-semibold hover:underline">
              Contatar suporte →
            </button>
          </div>
        </div>
